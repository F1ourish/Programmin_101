...
int [,] MultiplyMatrix(int[,] firstMatrix, int[,] secondMatrix, int[,] resultMatrix)
{
    for (int i = 0; i < massiveResult.GetLength(0); i++)
    {
        for (int j = 0; j < massiveResult.GetLength(1); j++)
        {
            ...
            {
                mult += massiveFirst[i, k] * massiveSecond[k, j];
            }
...
int[,] firstMatrix = DoDoubleMassiveFirst(rowsFirst, columnsAndRows, 0, 9);
PrintDoubleMassiveFirst(massiveFirst);
Console.WriteLine();
int[,] secondMatrix = DoDoubleMassiveSecond(columnsAndRows, columnsSecond, 0, 9);
PrintDoubleMassiveSecond(massiveSecond);
Console.WriteLine();

int[,] massiveResult = DoDoubleMassive(rowsFirst, columnsSecond, 0, 9);
MultiplyMatrix(massiveFirst, massiveSecond, massiveResult);
